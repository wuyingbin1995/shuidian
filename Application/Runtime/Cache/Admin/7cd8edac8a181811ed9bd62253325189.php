        ...
        <div class="panel">
            ...
            <div class="panel-body">
                <table id="demo-foo-addrow" class="table table-bordered table-hover toggle-circle" data-page-size="15">
                    ...
                    <?php if(is_array($auth)): $k = 0; $__LIST__ = $auth;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vol): $mod = ($k % 2 );++$k;?><tr>
                            <td><?php echo ($vol["role_id"]); ?></td>
                            <td><?php echo ($vol["role_name"]); ?></td>
                            <td><?php echo ($vol["role_auth_idname"]); ?></td>
...
<div class="modal fade" id="myModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
            <!-- 一级权限循环 -->
            <?php if(is_array($top)): $i = 0; $__LIST__ = $top;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$top_v): $mod = ($i % 2 );++$i;?><div class="form-horizontal">
                    <div class="row">
                        <div class="col-md-3">
                            <label class="form-checkbox form-normal form-primary form-text ">
                                <input type="checkbox"  class="InputZi" value="<?php echo ($top_v["id"]); ?>" disabled name="InputFU<?php echo ($top_v["id"]); ?>" >
                                <?php echo ($top_v["auth_name"]); ?>
                            </label>
                        </div>
                    </div>
                    <div class="row">
                    <!-- 二级权限循环 -->
                    <?php if(is_array($second)): $k = 0; $__LIST__ = $second;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$second_v): $mod = ($k % 2 );++$k; if($top_v["id"] == $second_v["pid"] ): ?><div class="col-md-offset-1 col-md-3">
                            <label  class="form-checkbox form-normal form-primary form-text " >
                                <input  type="checkbox" class="InputZi InputFU<?php echo ($top_v["id"]); ?>" disabled  value="<?php echo ($second_v["id"]); ?>" name="" ><?php echo ($second_v["auth_name"]); ?>
                            </label>
...
<script>
    $(function(){
        ...
        $('.demo-btn-addrow').click(function(){
            ...
            $.ajax({
                'url'   : '/shuidian/index.php/Admin/Role/role_add',
                'type'  : 'post',
                'data'  : data ,
                'dataType' : 'json',
       ...
       var   ejqx=$('#ejqx').html();
        ...
        $(".demo-modify-row").click(function(){
            ...
            $.ajax({
                'url'   : '/shuidian/index.php/Admin/Role/role_list',
                'type'  : 'post',
                'data'  : data ,
                'dataType' : 'json',
                'success' : function(response)
            ...
            $("#myModalSuccess").click(function(){
                ...
                //发送ajax添加请求
                $.ajax({
                    'url'   : '/shuidian/index.php/Admin/Role/role_edit',
                    'type'  : 'post',
                    'data'  : data,
                    'dataType' : 'json',
                    'success' : function(response){
        ...
        $(".demo-delete-row").click(function(){
            if(confirm('是否删除？'))
            {
                ...
                //发送ajax请求
                $.ajax({
                    'url':'/shuidian/index.php/Admin/Role/role_del',
                    'type':'post',
                    'data':data,
                    'dataType':'json',
                    'success':function(response){
